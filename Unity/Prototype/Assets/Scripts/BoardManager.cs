...
public class BoardManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int x=0; x< boardWidth; x++)
        {
            for (int z = 0; z < boardLength; z++)
            {
                ...
                position.x = x;
                position.z = z;
                CreateTile(position, isMatDark);
            }
        }

        // Randomly place player 1s
        BoardAddPieces(boardArray: player1s, num: numPlayers, minX: 0,                        maxX: playerSpace, minZ: 0, maxZ: boardLength, obj: player1PF, rand: rnd);
        BoardAddPieces(boardArray: player2s, num: numPlayers, minX: boardWidth - playerSpace, maxX: boardWidth,  minZ: 0, maxZ: boardLength, obj: player2PF, rand: rnd);
        BoardAddPieces(boardArray: boxes,    num: numBoxes,   minX: 0,                        maxX: boardWidth,  minZ: 0, maxZ: boardLength, obj: boxPF,     rand: rnd);
    }
    ...
    public void CreateTile(Vector3 pos, bool isDark = false)
    {
    ...
    public void BoardAddPieces(bool[,] boardArray, int num, int minX, int maxX, int minZ, int maxZ, GameObject obj, System.Random rand, float objHeight = 1f, int maxLoops = 500)
    {
        int cnt = 0;
        int cntLoops = 0;
        int posX;
